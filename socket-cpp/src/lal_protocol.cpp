...
std::unique_ptr<GBuffer> sock_packetMaker::build_message_packet(message_type_t type, const void *payload_ptr, int payload_length)
{
    std::unique_ptr<GBuffer> buff_ptr(new GBuffer(LENGTH_FULL_HEADER + payload_length));
    set_header(buff_ptr->m_ptr, TYPE_MESSAGE, LENGTH_PAYLOAD_HEADER + payload_length);
    message_header_t *header_ptr = (message_header_t *)(buff_ptr->m_ptr + LENGTH_MAIN_HEADER);
...
int sock_packetMaker::set_header(void *buff_ptr, header_type_t msg_type, int payload_length, bool ack)
{
    header_t *header_ptr = (header_t *)buff_ptr;
    struct timeval current;
    gettimeofday(&current, NULL);
    uint64_t timestamp = (uint64_t)(current.tv_sec) * 1000000 + (uint64_t)current.tv_usec;
...
header_type_t sock_packetMaker::parse_packet(const void *packet_ptr, int packet_len)
{
    int len = packet_len;
    ...
    while (len > LENGTH_MAIN_HEADER) {
        ...
        case TYPE_MESSAGE: {
            ...
            if (header_ptr->msg_size != (uint32_t)(message_size + LENGTH_MAIN_HEADER)) {
                DBG("message length check failed, header_size is %d, message_size is %d", header_ptr->msg_size, message_size);
                break;
            }
            return TYPE_MESSAGE;
        }
...
int SockServerManager::send_data(const sock_client_proxy_t *client, const void *data, size_t len)
{
...
int SockServerManager::send_message_packet(const sock_client_proxy_t *client, message_type_t type, const void *payload_ptr, int payload_len)
{
    if (client == nullptr || m_server == nullptr) return -1;
    std::unique_ptr<GBuffer> buff = m_maker->build_message_packet(type, payload_ptr, payload_len);
    return send_data(client, buff->m_ptr, (size_t)buff->m_length);
}

int SockServerManager::recv_data(sock_client_proxy_t *client, void *data, size_t len)
{
...
{
    if (sock_server_client_readable(m_server, 3) != true)
        return -1;
    for (int i = 0; i < SOCK_MAX_CLIENTS; i++) {
...
int SockClinetManager::send_message_packet(message_type_t type, const void *payload_ptr, int payload_len)
{
