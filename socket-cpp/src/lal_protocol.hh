...
public:
    sock_packetMaker(){};
    ~sock_packetMaker(){};
    std::unique_ptr<GBuffer> build_message_packet(message_type_t type, const void *payload_ptr, int payload_len);
    header_type_t parse_packet(const void *packet_ptr, int packet_len);

private:
    sock_packetMaker(const sock_packetMaker &);
    sock_packetMaker &operator=(const sock_packetMaker &)
    {
        return *this;
    }
    int set_header(void *buff_p, header_type_t msg_type, int payload_len, bool ack = false);
};

class SockServerManager
{
public:
    typedef std::function<void(SockServerManager *server, sock_client_proxy_t *client)> listener_callback_t;
    typedef std::function<void(SockServerManager *server, sock_client_proxy_t *client)> connected_callback_t;
    typedef std::function<void(SockServerManager *server, sock_client_proxy_t *client)> disconnect_callback_t;
    ...
    int send_data(const sock_client_proxy_t *client, const void *data, size_t len);
    ...
    int send_message_packet(const sock_client_proxy_t *client, message_type_t type, const void *payload_ptr, int payload_len);
...
private:
    ...
    SockServerManager &operator=(const SockServerManager &)
    {
        return *this;
    };
    static void task(SockServerManager *ptr)
    {
        ptr->mainloop();
    }
    void mainloop();
...
public:
    ...
    SockClinetManager(std::string sockaddr, int port, int type = SOCK_STREAM, float timeout = 2) :
    ...
    sock_client_t *get_sock_client()
    {
        return m_client;
    }

    int send_message_packet(message_type_t type, const void *payload_ptr, int payload_len);
...
private:
    ...
    SockClinetManager &operator=(const SockClinetManager &)
    {
        return *this;
    };
    static void task(SockClinetManager *ptr)
    {
        ptr->mainloop();
    }
    void mainloop();
