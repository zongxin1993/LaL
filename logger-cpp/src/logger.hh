...
class Logger {
 ...
 public:
  ...
  bool set_ModuleLevel(const LALLog::LogFlags module,
                       const LALLog::Level level);
  Level get_ModuleLevel() const { return m_log_level.load(); }

  void EnableModule(LALLog::LogFlags module) { m_module |= module; }
  void DisableModule(LALLog::LogFlags module) { m_module &= ~module; }
  std::mutex &get_mutex() { return m_lock_; }
...
static inline bool is_AcceptModule(LALLog::LogFlags module,
                                   LALLog::Level level) {
...
static inline void LogPrintf_(const std::string &logging_function,
                              ...
                              const LALLog::LogFlags flag,
                              const LALLog::Level level, const char *fmt,
                              const Args &...args) {
...
#define LogPrintLevel_(module, level, ...) \
  LogPrintf_(__func__, __FILE__, __LINE__, module, level, __VA_ARGS__)
