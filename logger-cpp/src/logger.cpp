...
std::string LALLog::Logger::LogTimestampStr(const std::string &str) {
  ...
  if (m_started_new_line) {
    ...
    res = str_format("%02d:%02d:%02d.%03d", tLocal.tm_hour, tLocal.tm_min,
                     tLocal.tm_sec, (int)(millsec.count() % 1000));
    res += str;
  } else {
    res = str;
  }
...
LALLog::Logger &LogInstance() {
   ...
   * work since the order of destruction of static/global objects is undefined.
...
void LALLog::Logger::LogPrintStr(const std::string &str,
                                 ...
                                 int source_line, LALLog::LogFlags module,
                                 LALLog::Level level) {
...
bool LALLog::Logger::is_ModuleLevel(LALLog::LogFlags module,
                                    LALLog::Level level) const {
  ...
  bool res = level >=
             (it == m_module_log_levels.end() ? get_ModuleLevel() : it->second);

  return level >=
         (it == m_module_log_levels.end() ? get_ModuleLevel() : it->second);
}

bool LALLog::Logger::set_ModuleLevel(const LALLog::LogFlags module,
